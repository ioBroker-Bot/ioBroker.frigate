...
interface TrackedEvent {
    label: string;
    stationary: boolean;
    /** Name Frigate recognized for the object (face, known plate), '' while there is none */
    subLabel: string;
}
...
export class ZoneAggregator {
     ...
     */
    static getSubLabel(eventData: { sub_label?: unknown }): string {
     ...
     */
    async resetSubLabels(): Promise<void> {
        ...
        try {
            ...
            this.ctx.adapter.log.warn(
                `Cannot reset the recognized names: ${error instanceof Error ? error.message : String(error)}`,
            );
        }
    }
     ...
     */
    async addKnownSubLabels(names: string[]): Promise<void> {
    ...
    async initZones(configData: any): Promise<void> {
        ...
        for (const zone of zones) {
            ...
            await this.createZoneState(
                `${zone}.sub_labels`,
                `Names recognized in zone ${zone} (faces, known plates), comma separated`,
                'string',
                'text',
                '',
            );
     ...
     */
    private async updateSubLabels(eventId: string, subLabel: string): Promise<void> {
        if (subLabel) {
     ...
     */
    private async writeSubLabel(id: string, name: string, value: boolean): Promise<void> {
        if (!this.subLabelStates.has(id)) {
     ...
     */
    private toStateId(name: string): string {
